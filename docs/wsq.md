...
- a reference-precision encoder-analysis path for `2.0 bpp` and above that uses double normalization, reference filter coefficients, double/FMA decomposition, and reference-precision variance and quantization math
- active exact regression coverage for the current `2.25 bpp` public-corpus coefficient set under the main quantization contract, which now passes on the higher-rate reference-precision path
...
- exact encoder classification across the 80 public encoder cases currently splits into 38 cases that match both the published NIST corpus and a current local NBIS build, 9 that match only the published NIST corpus, 10 that match only the local NBIS build, and 23 that match neither exactly
- because NIST's published WSQ variance-change notice documents a compression-path change introduced in NBIS 3.3.1, the repository treats the published NIST reference corpus as the primary certification-aligned parity target and the local NBIS build as a diagnostic reference rather than a second hard truth source
- the encoder internals are now split so WSQ value scaling, variance calculation, coefficient quantization, and quantization-bin synthesis can be investigated independently, and the remaining public-reference gap is now concentrated in the broader skipped exact coefficient contracts plus the unresolved `0.75 bpp` public-vs-NBIS divergence cases
- the active exact `2.25 bpp` regression coverage is currently green under the main quantization contract for the promoted public-corpus set, and the next three blocker cases are tracked under a separate focused gate with explicit mismatch diagnostics
