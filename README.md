...
The repository is still early, but it is no longer scaffolding-only. The current WSQ work now includes a managed parser, a managed decode path backed by the official NIST reference codestream corpus and NBIS-generated decoder reconstructions, and a working managed encode path that accepts raw grayscale input streams and emits WSQ codestreams with generated Huffman tables and NISTCOM metadata. The managed decoder now matches the local NBIS-generated raw reference reconstructions byte-for-byte across the official public decoder corpus. On the encoder side, the managed implementation now satisfies the published NIST public-corpus checks for file size, frame-header values, quantization-bin widths, and quantized coefficient-bin tolerances when encoded with the reference software implementation number used by the included corpus. Exact reference-codestream parity is still a stricter separate goal, and formal FBI/NIST certification is still outside the scope of the current repository state.
