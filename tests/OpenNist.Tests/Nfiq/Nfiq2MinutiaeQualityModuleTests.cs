...
internal sealed class Nfiq2MinutiaeQualityModuleTests
{
    ...
    [Test]
    [DisplayName("should reproduce the official minutiae-quality features for every bundled SFinGe image")]
    [MethodDataSource(typeof(Nfiq2TestDataSources), nameof(Nfiq2TestDataSources.ExampleCases))]
    public async Task ShouldReproduceTheOfficialMinutiaeQualityFeaturesForEveryBundledSfinGeImage(Nfiq2ExampleCase exampleCase)
    {
        ...
        var managed = Nfiq2MinutiaeQualityModule.Compute(croppedImage, minutiae);
        ...
        {
            AssertApproximatelyEqual($"{exampleCase.Name} {featureName}", native.NativeQualityMeasures[featureName], managed[featureName]);
        }
    }
