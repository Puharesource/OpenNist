...
internal static class Nfiq2FingerJetImagePreparation
{
    ...
    {
        var validationErrors = new List<Nfiq2ValidationError>(capacity: 4);

        if (width > s_createFeatureSetMaxDimension || height > s_createFeatureSetMaxDimension)
        {
            validationErrors.Add(
                Nfiq2Errors.FingerJetCreateFeatureSetDimensionsExceeded(width, height, s_createFeatureSetMaxDimension));
        }

        if (pixelsPerInch is < s_createFeatureSetMinDpi or > s_createFeatureSetMaxDpi)
        {
            validationErrors.Add(
                Nfiq2Errors.FingerJetCreateFeatureSetResolutionOutOfRange(
                    pixelsPerInch,
                    s_createFeatureSetMinDpi,
                    s_createFeatureSetMaxDpi));
        }
        ...
        {
            validationErrors.Add(
                Nfiq2Errors.FingerJetCreateFeatureSetWidthOutOfRange(
                    width,
                    pixelsPerInch,
                    s_createFeatureSetMinSizeAt500Scale,
                    s_createFeatureSetMaxWidthAt500Scale));
        }
        ...
        {
            validationErrors.Add(
                Nfiq2Errors.FingerJetCreateFeatureSetHeightOutOfRange(
                    height,
                    pixelsPerInch,
                    s_createFeatureSetMinSizeAt500Scale,
                    s_createFeatureSetMaxHeightAt500Scale));
        }
    ...
    public static Nfiq2FingerJetPreparedImage Prepare(Nfiq2FingerprintImage image)
    {
        ...
        if (height > s_maxHeight)
        {
            ...
            if (diff <= 0)
            {
                throw Nfiq2Errors.ExceptionFrom(Nfiq2Errors.FingerJetCropPlanningDiverged("vertical"));
            }
        ...
        if (width > s_maxWidth)
        {
            ...
            if (diff <= 0)
            {
                throw Nfiq2Errors.ExceptionFrom(Nfiq2Errors.FingerJetCropPlanningDiverged("horizontal"));
            }
        ...
        {
            throw Nfiq2Errors.ExceptionFrom(Nfiq2Errors.FingerJetWorkingBufferExceeded(bufferSizeNeeded, s_maxBufferSize));
        }
    ...
    {
        var validationErrors = new List<Nfiq2ValidationError>(capacity: 5);

        if (image.Pixels.Length < checked(image.Width * image.Height))
        {
            validationErrors.Add(Nfiq2Errors.FingerJetInputPixelBufferTooSmall(image.Pixels.Length, checked(image.Width * image.Height)));
        }

        if (image.PixelsPerInch is < s_extractMinDpi or > s_extractMaxDpi)
        {
            validationErrors.Add(
                Nfiq2Errors.FingerJetExtractionResolutionOutOfRange(
                    image.PixelsPerInch,
                    s_extractMinDpi,
                    s_extractMaxDpi));
        }
        ...
        var heightAt500 = image.Height * 500 / image.PixelsPerInch;
        if (widthAt500 < s_extractMin500Width)
        {
            validationErrors.Add(Nfiq2Errors.FingerJetExtractionWidthTooSmall(widthAt500, s_extractMin500Width));
        }

        if (widthAt500 > s_extractMax500Width)
        {
            validationErrors.Add(Nfiq2Errors.FingerJetExtractionWidthTooLarge(widthAt500, s_extractMax500Width));
        }

        if (heightAt500 < s_extractMin500Height)
        {
            validationErrors.Add(Nfiq2Errors.FingerJetExtractionHeightTooSmall(heightAt500, s_extractMin500Height));
        }

        if (heightAt500 > s_extractMax500Height)
        {
            validationErrors.Add(Nfiq2Errors.FingerJetExtractionHeightTooLarge(heightAt500, s_extractMax500Height));
        }
