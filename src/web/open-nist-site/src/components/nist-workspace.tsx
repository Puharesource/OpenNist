...
function resolveSelection(
  document: NistWorkspaceDocument | null,
  selection: NistSelection | null
): ResolvedNistSelection | null {
...
async function renderRasterPreviewBytes(
  ...
  if (!context || !("createImageData" in context) || !("putImageData" in context)) {
      ...
      if (resolvedSelection.kind === "record") {
        ...
        if (!imageField) {
          setPreviewState({
            status: "unsupported",
            message:
              "Select a field to inspect its value, or select a record that contains an image payload to preview it."
          })
          return
        }
                        ...
                        {document.fileInfo.records.map((record, recordIndex) => {
                          ...
                          return (
                            <div
                              ...
                              className="ml-4 border-l-2 border-[color:var(--effect-ghost-border)]"
                            >
                                ...
                                <div className="ml-6 mt-1 space-y-1">
                                  {displayFields.length === 0 ? (
                                    <div className="rounded-[var(--radius-lg)] px-3 py-2 text-xs leading-5 text-[var(--color-on-surface-variant)]">
                                      This record is stored as opaque binary data. Select the record header to inspect
                                      or preview it.
                                    </div>
                                  ) : null}
                                        ...
                                        {hasSubfields && !isFieldCollapsed ? (
                                          <div className="ml-7 space-y-1 border-l border-[color:var(--effect-ghost-border)] pl-3">
                                            {field.subfieldItems.map((items, subfieldIndex) =>
                                              (() => {
                                                const subfieldKey = buildSubfieldKey(
                                                  recordIndex,
                                                  field.key,
                                                  subfieldIndex
                                                )
                                                ...
                                                return (
                                                  <div key={subfieldKey} className="space-y-1">
                                                    <button
                                                      ...
                                                      aria-label={
                                                        isSubfieldCollapsed ? "Expand subfield" : "Collapse subfield"
                                                      }
                                                    >
                                                      {isSubfieldCollapsed ? (
                  ...
                  <div className="mx-auto flex min-h-[500px] max-w-[520px] items-center justify-center px-8">
                    <div className="space-y-5 text-center">
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-sm leading-7 text-[var(--color-on-surface-variant)]">
                          Open a `.nist`, `.an2`, or `.eft` file to inspect logical records, field meanings, and image
                          payloads.
                        </p>
      ...
      {showRightSidebar && resolvedSelection ? (
        <NistInspector
          ...
          onDownloadBinaryAsset={() => {
            ...
            void downloadBytes(binaryAsset.bytes, binaryAsset.fileName, binaryAsset.mimeType)
          }}
...
function NistInspector({
  ...
  const fieldMetadata =
    selection.kind === "field" ? getNistFieldMetadata(selection.record.type, selection.field.fieldNumber ?? -1) : null
  const inspectorTitle = selection.kind === "field" ? selection.field.tag : `Type-${selection.record.type}`
  ...
  const showBinaryPreviewSection =
    isPreviewableBinarySelection &&
    (previewState.status === "loading" || previewState.status === "ready" || previewState.status === "error")

  const recordRows = [
    {
      label: "Record length",
      value: selection.record.logicalRecordLength
        ? `${selection.record.logicalRecordLength}`
        : `${selection.record.encodedByteCount}`,
  ...
  const fieldRows =
    selection.kind === "field"
      ? [
          {
            label: "Raw length",
            value: selection.field.binaryBytes
              ? formatByteSize(selection.field.binaryBytes.length)
              : `${selection.field.value.length} chars`,
    ...
    >
      ...
      {selection.kind === "field" ? (
        <>
          ...
          <InspectorSection title="Field details" items={fieldRows} />
        </>
      ) : (
