...
function describeQualityMeasure(key: string): string | undefined {
  ...
  const descriptions: Record<string, string> = {
    ...
    MMB:
      "Displayed here as a normalized 0-100 score, derived from a raw 0-255 brightness measure. Mid-range values are usually healthier; very low or very high brightness can make ridge detail harder to read.",
      ...
      try {
        if (fileName.toLowerCase().endsWith(".wsq")) {
          const loadedDocument = await withBusy(
            "Decoding WSQ and computing its NFIQ 2 score.",
            async () => {
